...
function renderDecisionReview(){return st.decisions.map(d=>{const shown=d.replacement?`${label(d.action)} → ${label(d.replacement)}`:label(d.action),a=actionAnalysis(d.action,'customer'),out=decisionOutcome(d);let consequence='The negotiation continued.';let options='';if(d.round===1&&st.g.terminal?.[d.action]){consequence=`The negotiation ended immediately with ${label(st.g.terminal[d.action])}.`;options='All later negotiation options were closed.'}else if(d.round===1){options='The student retained access to later negotiation rounds.'}else if(d.round===3){consequence=out?`If preserved, this action would produce ${label(out)} with a customer score of ${outcomeScore(out)}.`:'The action created an intermediate state and preserved further negotiation.';options=`Other valid choices at the time: ${d.available.filter(x=>x!==d.action).map(label).join(', ')||'None'}.`}else if(d.round===4){consequence=`You offered ${label(d.action)} as the value exchange before the supplier's next response.`;options=`Other concession choices available: ${d.available.filter(x=>x!==d.action).map(label).join(', ')||'None'}.`; }else if(d.round===6){consequence=`The decision made ${label(out)} the final outcome.`;options=d.action==='commit'?'The current outcome was preserved.':`Commit would have preserved ${label(resolveCurrentOutcome())}.`}const rating=ratingForDecision(d);return `<div class="analysis-card"><div class="analysis-card-head"><strong>Round ${d.round}: ${shown}</strong><span class="rating-badge ${ratingClass(rating)}">${rating}</span></div><p><b>Benefit:</b> ${a?.benefit||'The decision changed the available negotiation path.'}</p><p><b>Risk:</b> ${a?.risk||'The decision may reduce later options.'}</p><p><b>Decision consequence:</b> ${consequence}</p><p><b>Options affected:</b> ${options}</p><p><b>Advice:</b> ${a?.advice||'Compare the decision with the strongest valid alternative using the same information available at the time.'}</p></div>`}).join('')}
