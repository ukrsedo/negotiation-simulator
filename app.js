...
function playRound(){
  ...
  if(st.round===4){if(st.key==='SAP_Transformation')return sapConcessionRound();if(st.key==='Oracle_Audit')return oracleConcessionRound()}
...
function sapRatingPercent(rating){
...
function score(){const o=st.finalOutcome,ach=st.g.objective_achievement[o]||{customer:[],supplier:[]};const cs=(ach.customer||[]).reduce((n,x)=>n+(st.g.customer_objective_points[x]||0),0);const ss=(ach.supplier||[]).reduce((n,x)=>n+(st.g.profiles[st.profile].hidden_distribution[x]||0),0);const ce=st.g.customer_economic_scores[o]??0,se=st.g.supplier_economic_scores[o]??0,cm=concessionModifier();const cf=st.key==='Oracle_Audit'?oracleDecisionScore():st.key==='SAP_Transformation'?sapDecisionScore():Math.max(0,Math.min(100,cs+ce+cm));return {ach,cs,ss,ce,se,cm,cf,sf:ss+se}}
