...
function roundProgress(round){
  return `<div class="round-progress" aria-label="Round ${round} of 5">${[1,2,3,4,5].map(n=>`<span class="round-step ${n<round?'done':n===round?'current':''}"><b>${n}</b><small>${n===round?'Current':''}</small></span>`).join('')}</div>`;
}
...
function home(){
  ...
  set(`<div class="screen-head"><div><p class="section-kicker">Choose a scenario</p><h2>Negotiation simulations</h2><p>Select a case and negotiate through five rounds. Your customer result can contribute up to 10 course points.</p></div></div><div id="scenario-grid" class="scenario-grid"></div>`);
  const grid=document.getElementById('scenario-grid');
  Object.entries(S).forEach(([k,g],i)=>{
    const card=document.createElement('button');
    card.className='scenario-card';
    card.innerHTML=`<span class="scenario-no">${String(i+1).padStart(2,'0')}</span><span class="scenario-type">5-round simulation · up to 10 course points</span><strong>${g.game.name}</strong><span class="scenario-desc">${g.brief}</span><span class="scenario-cta">Start simulation <b>→</b></span>`;
...
function playRound(){
  if(st.round===2||st.round===4){supplierTurn();return}
  const r=st.g.rounds[`round_${st.round}`];
  set(`<div class="round-head"><div><p class="section-kicker">${st.g.game.name}</p><h2>Round ${st.round} of 5 · Your decision</h2></div></div>${roundProgress(st.round)}
    <section class="action-section"><h3>Available actions</h3><p>Choose the action you would take at this point in the negotiation.</p><div id="available" class="action-grid"></div></section>
...
function supplierTurn(){
  ...
  if(st.round===4&&st.round3==='negotiate_concession')prefKey='round_4_after_negotiate_concession';
  ...
  set(`<div class="round-head"><div><p class="section-kicker">${st.g.game.name}</p><h2>Round ${st.round} of 5 · Supplier response</h2></div></div>${roundProgress(st.round)}
...
function chooseReplacement(){
  ...
  set(`<div class="round-head"><div><p class="section-kicker">${st.g.game.name}</p><h2>Round 5 of 5 · Change direction</h2></div></div>${roundProgress(5)}<section class="action-section"><h3>Available outcomes</h3><div id="replacement-grid" class="action-grid"></div></section><details class="blocked-panel"><summary>Unavailable outcomes <span id="replacement-blocked-count"></span></summary><div id="replacement-blocked"></div></details>`);
  ...
  Object.entries(r3).forEach(([a,d])=>{
    ...
    if(v.ok){
      ...
      b.onclick=()=>{const drec=st.decisions[st.decisions.length-1];drec.replacement=a;drec.replacementAvailable=[...availableActionsForReplacement()];st.history.push({round:5,actor:'Customer',action:`change_direction → ${a}`});st.finalOutcome=a;results()};grid.appendChild(b);
...
function renderDecisionReview(){return st.decisions.map(d=>{const shown=d.replacement?`${label(d.action)} → ${label(d.replacement)}`:label(d.action),a=actionAnalysis(d.action,'customer'),out=decisionOutcome(d);let consequence='The negotiation continued.';let options='';if(d.round===1&&st.g.terminal?.[d.action]){consequence=`The negotiation ended immediately with ${label(st.g.terminal[d.action])}.`;options='All later negotiation options were closed.'}else if(d.round===1){options='The student retained access to later negotiation rounds.'}else if(d.round===3){consequence=out?`If preserved, this action would produce ${label(out)} with a customer score of ${outcomeScore(out)}.`:'The action created an intermediate state and preserved further negotiation.';options=`Other valid choices at the time: ${d.available.filter(x=>x!==d.action).map(label).join(', ')||'None'}.`}else if(d.round===5){consequence=`The decision made ${label(out)} the final outcome.`;options=d.action==='commit'?'The current outcome was preserved.':`Commit would have preserved ${label(resolveCurrentOutcome())}.`}const rating=ratingForDecision(d);return `<div class="analysis-card"><div class="analysis-card-head"><strong>Round ${d.round}: ${shown}</strong><span class="rating-badge ${ratingClass(rating)}">${rating}</span></div><p><b>Benefit:</b> ${a?.benefit||'The decision changed the available negotiation path.'}</p><p><b>Risk:</b> ${a?.risk||'The decision may reduce later options.'}</p><p><b>Decision consequence:</b> ${consequence}</p><p><b>Options affected:</b> ${options}</p><p><b>Advice:</b> ${a?.advice||'Compare the decision with the strongest valid alternative using the same information available at the time.'}</p></div>`}).join('')}
...
function results(){
  const q=score();st.q=q;const classification=classify(q.cf,q.sf),courseScore=Math.round(q.cf/10);
  set(`<div class="results-hero"><p class="section-kicker">Simulation complete</p><h2>Negotiation Result — ${classification}</h2><div class="result-meta"><span><small>Final outcome</small><strong>${label(st.finalOutcome)}</strong></span><span><small>Course score earned</small><strong>${courseScore}/10</strong></span></div><p>${classificationNote(classification,q.cf,q.sf)}</p></div>
    <div class="score-grid"><div class="score-card customer-score"><span>Customer</span><strong>${q.cf}<small>/100</small></strong><p>Strategic ${q.cs} · Economic ${q.ce}</p><small>Objectives: ${q.ach.customer.map(label).join(', ')||'None'}</small></div><div class="score-card supplier-score"><span>Supplier</span><strong>${q.sf}<small>/100</small></strong><p>Strategic ${q.ss} · Economic ${q.se}</p><small>Objectives: ${q.ach.supplier.map(label).join(', ')||'None'}</small></div></div>
